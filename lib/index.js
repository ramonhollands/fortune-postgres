...
module.exports = Adapter => class PostgreSQLAdapter extends Adapter {
  ...
  connect () {
    ...
    // This is overridden with a single client instance during a transaction.
    ...
    return (connection ?
      ...
      .then(() => Promise.all(types.map(type =>
        new Promise((resolve, reject) => {
          ...
          // Noise reduction.
          this.pool.query('set client_min_messages = warning',
      ...
      (hasPrimaryKey ? [ `"${primaryKey}"` ] : [])
        ...
        records.map(record => {
          ...
          return `(${(hasPrimaryKey ?
            [ primaryKey ].concat(orderedFields) : orderedFields)
            .map(() => {
              index++
              return `$${index}`
            }).join(', ')})`
